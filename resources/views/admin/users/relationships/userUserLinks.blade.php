@can('user_link_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.user-links.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.userLink.title_singular') }}
            </a>
        </div>
    </div>
@endcan
                ...
                <tbody>
                    @foreach($userLinks as $key => $userLink)
                        <tr data-entry-id="{{ $userLink->id }}">
                            ...
                            <td>
                                @can('user_link_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.user-links.show', $userLink->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('user_link_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.user-links.edit', $userLink->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('user_link_delete')
                                    <form action="{{ route('admin.user-links.destroy', $userLink->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_link_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.user-links.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-userUserLinks:not(.ajaxTable)').DataTable({ buttons: dtButtons })
