...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-menuClientListMenuCategories">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $menuCategory->id }}">
                            ...
                            <td>
                                @can('menu_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.menu-categories.show', $menuCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('menu_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.menu-categories.edit', $menuCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('menu_category_delete')
                                    <form action="{{ route('admin.menu-categories.destroy', $menuCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-menuClientListMenuCategories:not(.ajaxTable)').DataTable({ buttons: dtButtons })
