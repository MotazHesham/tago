    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.templates.update", [$template->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="text" name="price" id="price" value="{{ old('price', $template->price) }}" required>
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                <label for="canvas_pages">{{ trans('cruds.template.fields.canvas_pages') }}</label>
                <textarea class="form-control {{ $errors->has('canvas_pages') ? 'is-invalid' : '' }}" name="canvas_pages" id="canvas_pages">{{ old('canvas_pages', $template->canvas_pages) }}</textarea>
                @if($errors->has('canvas_pages'))
                    <div class="invalid-feedback">
                        {{ $errors->first('canvas_pages') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.template.fields.canvas_pages_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    init: function () {
@if(isset($template) && $template->photo)
      var file = {!! json_encode($template->photo) !!}
