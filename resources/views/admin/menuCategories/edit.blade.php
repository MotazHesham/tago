...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.menuCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.menu-categories.update", [$menuCategory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.menuCategory.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $menuCategory->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('menu_client') ? 'is-invalid' : '' }}" name="menu_client_id" id="menu_client_id" required>
                    @foreach($menu_clients as $id => $entry)
                        <option value="{{ $id }}" {{ (old('menu_client_id') ? old('menu_client_id') : $menuCategory->menu_client->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('menu_client_lists') ? 'is-invalid' : '' }}" name="menu_client_lists[]" id="menu_client_lists" multiple required>
                    @foreach($menu_client_lists as $id => $menu_client_list)
                        <option value="{{ $id }}" {{ (in_array($id, old('menu_client_lists', [])) || $menuCategory->menu_client_lists->contains($id)) ? 'selected' : '' }}>{{ $menu_client_list }}</option>
                    @endforeach
                </select>
