...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.menuPackage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.menu-packages.update", [$menuPackage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.menuPackage.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $menuPackage->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('descrption') ? 'is-invalid' : '' }}" name="descrption" id="descrption">{!! old('descrption', $menuPackage->descrption) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $menuPackage->price) }}" step="0.01">
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('menus') ? 'is-invalid' : '' }}" type="number" name="menus" id="menus" value="{{ old('menus', $menuPackage->menus) }}" step="0.01">
                @if($errors->has('menus'))
                    <div class="invalid-feedback">
                        {{ $errors->first('menus') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('themes') ? 'is-invalid' : '' }}" name="themes[]" id="themes" multiple required>
                    @foreach($themes as $id => $theme)
                        <option value="{{ $id }}" {{ (in_array($id, old('themes', [])) || $menuPackage->themes->contains($id)) ? 'selected' : '' }}>{{ $theme }}</option>
                    @endforeach
                </select>
