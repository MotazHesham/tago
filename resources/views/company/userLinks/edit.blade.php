...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('company.user-links.update', [$userLink->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('main_link') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($main_links as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('main_link_id') ? old('main_link_id') : $userLink->main_link->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                        name="name" id="name" value="{{ old('name', $userLink->name) }}" required>
                    @if ($errors->has('name'))
                        <div class="invalid-feedback">
                            {{ $errors->first('name') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text"
                        name="link" id="link" value="{{ old('link', $userLink->link) }}" required>
                    @if ($errors->has('link'))
                        <div class="invalid-feedback">
                            {{ $errors->first('link') }}
                        </div>
                ...
                <div class="form-group">
                    <label class="required" for="priority">{{ trans('cruds.userLink.fields.priority') }}</label>
                    <input class="form-control {{ $errors->has('priority') ? 'is-invalid' : '' }}" type="text"
                        name="priority" id="priority" value="{{ old('priority', $userLink->priority) }}" required>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="checkbox" name="active" id="active" value="1"
                            {{ $userLink->active || old('active', 0) === 1 ? 'checked' : '' }} required>
                        <label class="required form-check-label"
                            for="active">{{ trans('cruds.userLink.fields.active') }}</label>
                    </div>
