...
@section('content')
    <div class='container'>
        <div class='card'>
            ...
            <form method="POST" action="{{ route('menuClient.update_settings') }}" enctype="multipart/form-data" style="padding: 20px"> 
                @csrf
                <input type="hidden" name="user_id" value="{{ $menuClient->user_id }}" id="">
                <div class="row">
                    <div class="form-group col-md-4">
                        <label class="required" for="name">{{ trans('cruds.user.fields.name') }}</label>
                        <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                            name="name" id="name" value="{{ old('name', $menuClient->user->name) }}" required>
                        @if ($errors->has('name'))
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email"
                            name="email" id="email" value="{{ old('email', $menuClient->user->email) }}" required>
                        @if ($errors->has('email'))
                            <div class="invalid-feedback">
                                {{ $errors->first('email') }}
                            </div>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" type="text"
                            ...
                            value="{{ old('phone_number', $menuClient->user->phone_number) }}">
...
@section('scripts')
    <script>
        Dropzone.options.logoDropzone = {
            url: '{{ route('menuClient.settings.storeMedia') }}',
            maxFilesize: 2, // MB
                            ...
                            return loader.file
                                .then(function(file) {
                                    return new Promise(function(resolve, reject) {
                                        ...
                                        xhr.open('POST',
                                            '{{ route('menuClient.settings.storeCKEditorImages') }}',
                                            true);
