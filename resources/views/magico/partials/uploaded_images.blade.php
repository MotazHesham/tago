...
<div class="off-canvas-upload off-canvas-images hover-image" id="off-canvas-upload-{{$image->id}}">
    <img class="add-to-canvas"  src="{{ $image->mime_type == 'image/svg+xml' ? $image->getUrl() : $image->getUrl('preview2') }}" alt="" data-src="{{ $image->getUrl() }}" data-id="off-canvas-upload-{{$image->id}}"> 
